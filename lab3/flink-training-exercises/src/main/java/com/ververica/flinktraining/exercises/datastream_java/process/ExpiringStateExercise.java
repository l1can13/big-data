...
public class ExpiringStateExercise extends ExerciseBase {
	...
	public static void main(String[] args) throws Exception {
		...
		final int servingSpeedFactor = 600;      // 10 minutes of events are served every second
		...
		// Create data streams for rides and fares
		...
		// Connect and process the streams with the EnrichmentFunction
		SingleOutputStreamOperator<Tuple2<TaxiRide, TaxiFare>> processed = rides
				.connect(fares)
				.process(new EnrichmentFunction());

		// Print or test unmatched fares output
		printOrTest(processed.getSideOutput(unmatchedFares));

		// Execute the job
		env.execute("State Expiration Analysis (Java)");
	}
